...
class TSSIVAE(L.LightningModule):
    ...
    sigma_x_max: float = 1.0
    sigma_x_min: float | None = None
    sigma_x_step_start: int | None = 0
    sigma_x_step_end: int | None = 1
    sigma_x_mode: str = "FIXED"
    ...
    def __post_init__(self):
        ...
        self.sigma_x_min = self.sigma_x_min or self.sigma_x_max
        ...
        self.sequence_encoder_conv = torch.nn.Sequential(
            torch.nn.Conv2d(
                in_channels=self.cnn_block_sizes[-1] * self.cnn_block_width,
                out_channels=self.cnn_block_sizes[-1] * self.cnn_block_width * 2,
                kernel_size=(self.latent_window_length, self.latent_frequency_dim),
                stride=(self.latent_window_length, self.latent_frequency_dim),
            ),
        )
        self.sequence_encoder_mlp = torch.nn.Sequential(
            torch.nn.Linear(
                in_features=self.cnn_block_sizes[-1] * self.cnn_block_width * 2 * 2,
                out_features=self.cnn_block_sizes[-1] * self.cnn_block_width,
            ),
            Activation[self.mlp_activation].init(),
            torch.nn.Linear(
                in_features=self.cnn_block_sizes[-1] * self.cnn_block_width,
                out_features=self.latent_dim * 2
            )
        )
        self.mu_residual = torch.nn.Linear(in_features=self.latent_dim, out_features=self.latent_dim)
        self.sigma_residual = torch.nn.Linear(in_features=self.latent_dim, out_features=self.latent_dim)
        self.sequence_decoder = torch.nn.Sequential(
            torch.nn.Linear(
                in_features=self.latent_dim,
                out_features=self.latent_frequency_dim * self.latent_window_length
            ),
            torch.nn.Unflatten(-1, (1, self.latent_window_length, self.latent_frequency_dim))
        )
        self._reset_cache()
    ...
    def forward(self, x: Tensor) -> Dict[str, Tensor]:
        ...
        # x_j is x_i chunked into independently translated frames, sample from the prior over translations
        ...
        q_z_i, delta_i, q_c_i = self.encode(x_i) # (bs, seq, ld)
        # encode posterior for translated frames separately
        U_j = self.cnn_encode(x_j)
        q_z_j = self.content_encode(U_j.unsqueeze(1)).view(q_z_i.size()) # (bs, seq, ld)
        delta_j = self.shift_encode(U_j.unsqueeze(1)) # (bs * seq, 1, 1)
        q_c_j = self.sequence_encode(U_j.unflatten(0, (x.size(0), -1)))
        # unpack gaussians
        mu_c_i, log_sigma_sq_c_i = q_c_i.chunk(2, dim=-1)
        mu_c_j, log_sigma_sq_c_j = q_c_j.chunk(2, dim=-1) # (bs, ld)
        mu_z_i, log_sigma_sq_z_i = q_z_i.chunk(2, dim=-1)
        mu_z_j, log_sigma_sq_z_j = q_z_j.chunk(2, dim=-1) # (bs, seq, ld)
        # conatenate for loss
        q_z = torch.cat([q_z_i, q_z_j], dim=0)
        q_c = torch.cat([q_c_i, q_c_j], dim=0)
        ...
        if self.cross_decode == "soft":
            ...
            mu_c = 1/2 * mu_c_i + 1/2 * mu_c_j
            log_sigma_sq_c = (1/2 * log_sigma_sq_c_i.exp() + 1/2 * log_sigma_sq_c_j.exp() + 1/4 * (mu_c_i - mu_c_j).pow(2)).log()
            # soft-cross decoding for the content
            mu_z = 1/2 * mu_z_i + 1/2 * mu_z_j
            log_sigma_sq_z = (1/2 * log_sigma_sq_z_i.exp() + 1/2 * log_sigma_sq_z_j.exp() + 1/4 * (mu_z_i - mu_z_j).pow(2)).log()
            # apply as a residual to condition z on c
            mu_z = mu_z + self.mu_residual(mu_c).unsqueeze(1)
            log_sigma_sq_z = log_sigma_sq_z + self.sigma_residual(mu_c).unsqueeze(1)
            # reparametrise
            c = Normal(mu_c, (0.5 * log_sigma_sq_c).exp()).rsample() # (bs, seq, ld)
            z = Normal(mu_z, (0.5 * log_sigma_sq_z).exp()).rsample() # (bs, seq, ld)
            # decode to feature maps
            U_hat = self.mlp_decode(z.flatten(end_dim=1)).unflatten(0, (x.size(0), -1)) # (bs, seq, ch, fr, fq)
            # project sequence-level representation and apply as residual
            h = self.sequence_decoder(c).unsqueeze(1)
            # reconstruct a contiguous sequence
            x_hat_i = self.cnn_decode((U_hat + h).flatten(end_dim=1), delta_i) # (bs, 1, fr * seq, fq)
            # and reconstruct independent translations
            x_hat_j = self.cnn_decode((U_hat + h).flatten(end_dim=1), delta_j) # (bs * seq, 1, fr, fq)
        elif self.cross_decode == "hard":
            c_i = Normal(mu_c_i, (0.5 * log_sigma_sq_c_i).exp()).rsample() # (bs, seq, ld)
            c_j = Normal(mu_c_j, (0.5 * log_sigma_sq_c_j).exp()).rsample() # (bs, seq, ld)
            ...
            # decode to feature maps
            U_hat_i = self.mlp_decode(z_i.flatten(end_dim=1)).unflatten(0, (x.size(0), -1)) # (bs * seq, ch, fr, fq)
            U_hat_j = self.mlp_decode(z_j.flatten(end_dim=1)).unflatten(0, (x.size(0), -1)) # (bs * seq, ch, fr, fq)
            # project sequence-level representation and apply as residual
            h_i = self.sequence_decode(c_i).unsqueeze(1)
            h_j = self.sequence_decode(c_j).unsqueeze(1)
            # reconstruct a contiguous sequence
            x_hat_i = self.cnn_decode((U_hat_j + h_j).flatten(end_dim=1), delta_i) # (bs, 1, fr * seq, fq)
            # and reconstruct independent translations
            x_hat_j = self.cnn_decode((U_hat_i + h_i).flatten(end_dim=1), delta_j) # (bs * seq, 1, fr, fq)
    ...
    def encode(self, x: Tensor, hop_length: int | None = None) -> Tensor:
        ...
        q_z = self.content_encode(x)
        delta = self.shift_encode(x)
        q_c = self.sequence_encode(x)
        return q_z, delta, q_c
    ...
    def content_encode(self, x: Tensor) -> Tensor:
        q_z = self.content_encoder(x.flatten(end_dim=1)).unflatten(dim=0, sizes=(x.size(0), x.size(1)))
        mu, log_sigma_sq = q_z.chunk(2, dim=-1)
        log_sigma_sq = soft_clip(log_sigma_sq, minimum=np.log(self.sigma_z_min ** 2))
        q_z = torch.cat([mu, log_sigma_sq], dim=-1)
        return q_z

    def shift_encode(self, x: Tensor) -> Tensor:
    ...
    def sequence_encode(self, x: Tensor) -> Tensor:
        h = self.sequence_encoder_conv(x.flatten(end_dim=1)).unflatten(dim=0, sizes=(x.size(0), x.size(1)))
        h = torch.cat([h.max(dim=1)[0], h.mean(dim=1)], dim=1)
        q_c = self.sequence_encoder_mlp(h.flatten(start_dim=1))
        mu, log_sigma_sq = q_c.chunk(2, dim=-1)
        log_sigma_sq = soft_clip(log_sigma_sq, minimum=np.log(self.sigma_z_min ** 2))
        q_c = torch.cat([mu, log_sigma_sq], dim=-1)
        return q_c

    def decode(self, z: Tensor, c: Tensor, delta: Tensor | None = None) -> Tensor:
        x = self.mlp_decode(z)
        h = self.sequence_decoder(c)
        x = self.cnn_decode(x + h, delta)
        return x
    ...
    def loss(
        ...
        **kwargs: Any,
        ...
        log_sigma_sq_x = torch.tensor(self.sigma_x_max).pow(2).log()
        ...
        # q_c has a standard normal prior
        dkl_c = gaussian_kl_divergence_standard_prior(q_c).sum(dim=-1).mean()
        losses.append(dkl_c)
        outputs |= dict(dkl_c=dkl_c)
        # q_z has an autoregressive prior
        dkl_z = torch.cat([
            gaussian_kl_divergence(q_z[:, t, :], p_z_t).unsqueeze(1)
            for t, p_z_t in autoregressive_prior(q_z, self.alpha)
        ], dim=1)
        dkl = dkl_z.sum(dim=-1).mean()
        losses.append(dkl)
        outputs |= dict(
            dkl_z=dkl,
            smooth_dkl_z=dkl_z[:, :, self.smooth_idx].detach().sum(dim=-1).mean(),
            non_smooth_dkl_z=dkl_z[:, :, self.non_smooth_idx].detach().sum(dim=-1).mean(),
        )
    ...
    def metrics(
        ...
        **kwargs: Any
        ...
        sigma_z_i = np.exp(0.5 * log_sigma_sq_z_i)
        mu_z_j, log_sigma_sq_z_j = (t.flatten(end_dim=-2).cpu().numpy() for t in q_z_j.chunk(2, dim=-1))
        sigma_z_j = np.exp(0.5 * log_sigma_sq_z_j)
        mu_c, log_sigma_sq_c = (t.cpu().numpy() for t in q_c.chunk(2, dim=-1))
        sigma_c = np.exp(0.5 * log_sigma_sq_c)
        mu_c_i, log_sigma_sq_c_i = (t.cpu().numpy() for t in q_c_i.chunk(2, dim=-1))
        sigma_c_i = np.exp(0.5 * log_sigma_sq_c_i)
        mu_c_j, log_sigma_sq_c_j = (t.cpu().numpy() for t in q_c_j.chunk(2, dim=-1))
        sigma_c_j = np.exp(0.5 * log_sigma_sq_c_j)
        delta = torch.cat([delta_i.flatten(end_dim=1).unsqueeze(1), delta_j])
        return dict(
            sigma_delta=self.delta_sigma_current(self.trainer.global_step),
            mu_c=wandb.Histogram(np_histogram=np.histogram(mu_c, range=[-5.0, 5.0])),
            sigma_c=wandb.Histogram(np_histogram=np.histogram(sigma_c, range=[-5.0, 5.0])),
            mu_c_i=wandb.Histogram(np_histogram=np.histogram(mu_c_i, range=[-5.0, 5.0])),
            sigma_c_i=wandb.Histogram(np_histogram=np.histogram(sigma_c_i, range=[0.0, 2.0])),
            mu_c_j=wandb.Histogram(np_histogram=np.histogram(mu_c_j, range=[-5.0, 5.0])),
            sigma_c_j=wandb.Histogram(np_histogram=np.histogram(sigma_c_j, range=[0.0, 2.0])),
            mu_z=wandb.Histogram(np_histogram=np.histogram(mu_z, range=[-5.0, 5.0])),
            sigma_z=wandb.Histogram(np_histogram=np.histogram(sigma_z, range=[-5.0, 5.0])),
            mu_z_i=wandb.Histogram(np_histogram=np.histogram(mu_z_i, range=[-5.0, 5.0])),
            sigma_z_i=wandb.Histogram(np_histogram=np.histogram(sigma_z_i, range=[0.0, 2.0])),
            mu_z_j=wandb.Histogram(np_histogram=np.histogram(mu_z_j, range=[-5.0, 5.0])),
            sigma_z_j=wandb.Histogram(np_histogram=np.histogram(sigma_z_j, range=[0.0, 2.0])),
