...
class TCVAEEMA(L.LightningModule):
    ...
    def alpha(self) -> torch.Tensor:
        return torch.cat([
            torch.ones(len(self.non_smooth_idx), device=self.device) * self.non_smooth_alpha,
            torch.linspace(self.smooth_alpha_min, self.smooth_alpha_max, len(self.smooth_idx), device=self.device)
        ])
    ...
    def forward(self, x: Tensor) -> Dict[str, Tensor]:
        ...
        return dict(
            ...
            z=z, q_z=q_z, q_z_i=q_z_i, q_z_j=q_z_j,
    ...
    def loss(
        ...
        **kwargs: Any,
        ...
        outputs |= dict(log_likelihood_x=-nll, mae_frame=mae_frame)
        ...
        # anchor q_z using an autoregressive prior which is a mixture of gaussians
        dkl = autoregressive_mixture_kl_divergence(q_z, self.alpha, num_samples=50, dim=[0, 1])
        # sum over latent dimension, mean over translations, sequence and batch
        dkl_total = dkl.sum(dim=-1)
        losses.append(dkl_total)
        outputs |= dict(
            dkl=dkl_total,
            smooth_dkl=dkl[self.smooth_idx].detach().sum(dim=-1),
            non_smooth_dkl=dkl[self.non_smooth_idx].detach().sum(dim=-1),
        )
        ...
        if batch_idx < 4 and len(self.validation_step_outputs):
            ...
            for i in range(num_samples):
                ...
                lc = plot_latent_time_series(
                    ...
                    np.linspace(1.0, 13.0, len(self.smooth_idx)),
