...
def main(
    ...
    results = pd.read_parquet(results_dir / "test_results.parquet")
    ...
    scores = score_summary[["model", "scope", *score_name_map.values()]]
    df = pd.concat([
        scores,
    ...
    for dataset in df.dataset.unique():
        ...
        for subset in df.loc[df["dataset"] == dataset, "subset"].unique():
            df1 = (
                ...
                .melt(id_vars=["model", "scope"])
                .pivot(index="model", columns=["scope", "variable"], values="value")
            )
